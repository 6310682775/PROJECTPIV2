...
class Input_model:
    def __init__(self, weights='yolov7.pt', download=True, source="", img_size=640, view_img=True,
                 save_txt=True, no_trace=True, colored_trk=True, loop='loop.json', nosave=False,
                 project='runs/detect', name='', exist_ok=True, device='', augment=True, conf_thres=0.6,
                 iou_thres=0.5, classes=None, agnostic_nms=True, update=True):
...
def detect(opt, count_boxes, save_img=False):
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if len(det):
                ...
                for track in tracks:
                    ...
                    check_enter_exit_loop(track, count_boxes)
...
def Detection_call():
    opt = Input_model(weights='yolov7.pt', download=True, source="video2.mp4", img_size=640, view_img=True,
                      save_txt=True, no_trace=True, colored_trk=True, loop='car_track/loop.json')
    ...
    with torch.no_grad():
        if opt.update:  # update all models (to fix SourceChangeWarning)
            for opt.weights in ['yolov7.pt']:
                detect(opt, count_boxes)
                strip_optimizer(opt.weights)
        else:
            detect(opt, count_boxes)

    return opt, count_boxes
...
#     parser.add_argument('--project', default='runs/detect', // output video
